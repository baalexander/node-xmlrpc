var http             = require('http')
  ...
  , marshallResponse = marshall.marshallResponse
  , marshallFault    = marshall.marshallFault
...
function Server(options, isSecure) {
  ...
  function handle_rpc(request, response) {
    Deserializer.deserializeMethodCall(request, function(error, methodName, params) {
      ...
      that.emit(methodName, null, params, function(error, value) {
        var xml
        if (error !== null) {
          xml = marshallFault(error)
        } else {
          xml = marshallResponse(value)
        }
  ...
  var httpServer = isSecure ? https.createServer(options, handle_rpc)
                            : http.createServer(handle_rpc)
