...
function Server(options, isSecure) {
  ...
  this._httpServer = isSecure ? https.createServer(options, handleMethodCall)
                            : http.createServer(handleMethodCall)


  process.nextTick(function() {
    this._httpServer.listen(options.port, options.host, function() {
      this.emit('listening')
    }.bind(this))
  }.bind(this))
  this.close = function(callback) {
    this._httpServer.once('close', callback)
