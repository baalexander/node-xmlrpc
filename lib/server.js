...
function Server(options, isSecure, onListening) {
	...
	function _deserializeMethodCallback(response, error, methodName, params) {
		if (that._events.hasOwnProperty(methodName)) {
			that.emit(methodName, null/*error*/, params, _callback.bind(null, response));
		}
			...
			that.emit(options.anyMethodName, null/*error*/, params, _callback.bind(null, response));
		}
		else {
	...
	function handleMethodCall(request, response) {
		...
		deserializer.deserializeMethodCall(request, _deserializeMethodCallback.bind(null, response));
	}
