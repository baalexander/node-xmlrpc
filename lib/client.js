 ...
 *   - {String} [encoding='utf8'] - (optional) - Set the encoding of the data sent by the client
...
function Client(options, isSecure) {
  ...
  options.method = 'POST'
  // Make a shallow-copy of the options, without 'encoding'
  this.options = Object.keys(options).reduce(function (obj, k) {
    if (["encoding"].indexOf(k) === -1) {
      obj[k] = options[k]
    }
...
Client.prototype.methodCall = function methodCall(method, params, callback) {
  ...
  options.headers['Content-Length'] = Buffer.byteLength(xml, this.encoding)
  ...
  request.write(xml, this.encoding)
  request.end()
}
