 ...
 */
exports.serializeMethodCall = function(method, params, forceSerializeAsText) {
  var params = params || []
  ...
  params.forEach(function(param) {
    serializeValue(param, xml.ele('param'), forceSerializeAsText)
  })
...
function serializeValue(value, xml, forceSerializeAsText) {
    ...
    if (current.index !== undefined) {
      ...
      switch(typeof current.value) {
        case 'boolean':
          ...
          break
        case 'string':
          appendString(current.value, valueNode, forceSerializeAsText)
          stack.pop()
          break
        case 'number':
...
var illegalChars = /^(?![^<&]*]]>[^<&]*)[^<&]*$/
function appendString(value, xml, forceSerializeAsText) {
  if (value.length === 0) {
    xml.ele('string')
  }
  else if (!illegalChars.test(value) && !forceSerializeAsText) {
