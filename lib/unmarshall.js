...
var Unmarshaller = exports.Unmarshaller = function Unmarshaller(cb) {
  ...
  self._parser.onopentag  = invoke(self, 'start')
  self._parser.onclosetag = invoke(self, 'end')
  self._parser.ontext     = invoke(self, 'text')
  self._callback = cb;
...
Unmarshaller.prototype.close = function() {
  ...
  if (this._responseType === null || this._marks.length) {
    this._callback(new Error('xmlrpc response error'))
    return
  }
