...
/**
 * Sets or gets the cookie by its name.
 * If value is not specified then returns the latest cookie value received from server with 'Set-Cookie' header.
 * If value is specified then it will be sent to the server during the next xml-rpc call. The method returns client itself,
 * so it is possible to chain calls like the following:
 *
 * <code>
 ...
 * @param {String?} value value to be set. If missed then current value will be returned
 * @return {*} cookie's value or client object itself
 */
Client.prototype.cookie = function cookie(name, value) {
  if (!this.cookies) {
    throw "Cookies support is not turned on for this client isntance";
  }
  var retValue = this.cookies.cookie(name, value);
  return retValue == this.cookies ? this : retValue;
}
